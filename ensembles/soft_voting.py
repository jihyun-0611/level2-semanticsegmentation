...
class EnsembleDataset(Dataset):
    ...
    def __init__(self, fnames, cfg, tf_dict):
        ...
        self.image_root = cfg.soft_voting.image_root
        self.tf_dict = tf_dict
        self.ind2class = {i : v for i, v in enumerate(cfg.soft_voting.CLASSES)}
...
def load_models(cfg, device):
    ...
    print("\n======== Model Load ========")
    print(cfg.soft_voting.model_paths.items())
    # inference 해야하는 이미지 크기 별로 모델 순차저장
    for key, paths in cfg.soft_voting.model_paths.items():
        if len(paths) == 0:
            continue
        model_dict[key] = []
        ...
        for path in paths:
            ...
            try:
                ...
                state_dict = torch.load(con.MODEL.SAVED_DIR+'/'+con.MODEL.MODEL_NAME)
...
def save_results(cfg, filename_and_class, rles):
    ...
    print("\n======== Save Output ========")
    print(f"{cfg.soft_voting.save_dir} 폴더 내부에 {cfg.soft_voting.output_name}을 생성합니다..", end="\t")
    os.makedirs(cfg.soft_voting.save_dir, exist_ok=True)

    output_path = osp.join(cfg.soft_voting.save_dir, cfg.soft_voting.output_name)
    try:
    ...
    print(f"{osp.join(cfg.soft_voting.save_dir, cfg.soft_voting.output_name)} 생성 완료")
...
def soft_voting(cfg):
    ...
    fnames = {
        osp.relpath(osp.join(root, fname), start=cfg.soft_voting.image_root)
        for root, _, files in os.walk(cfg.soft_voting.image_root)
        for fname in files
        if osp.splitext(fname)[1].lower() == ".png"
    }

    tf_dict = {image_size : A.Resize(height=image_size, width=image_size) 
               for image_size, paths in cfg.soft_voting.model_paths.items() 
               if len(paths) != 0}
    ...
    data_loader = DataLoader(dataset=dataset,
                             batch_size=cfg.soft_voting.batch_size,
                             shuffle=False,
                             num_workers=cfg.soft_voting.num_workers,
                             drop_last=False,
    ...
    with torch.no_grad():
        with tqdm(total=len(data_loader), desc="[Inference...]", disable=False) as pbar:
            for image_dict, image_names in data_loader:
                total_output = torch.zeros((cfg.soft_voting.batch_size, len(cfg.soft_voting.CLASSES), 2048, 2048)).to(device)
                ...
                total_output = (total_output > cfg.soft_voting.threshold).detach().cpu().numpy()
